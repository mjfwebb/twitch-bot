...
export function runTwitchIRCWebsocket() {
  ...
  client.on("connect", function (connection) {
    ...
    connection.on("error", function (error) {
      logger.error(
        "Twitch IRC WebSocket: Connection Error: " + error.toString(),
      );
    });
        ...
        messages.forEach((message) => {
          ...
          if (parsedMessage && parsedMessage.command) {
            ...
            switch (parsedMessage.command.command) {
              case "PRIVMSG":
                botCommandHandler(connection, parsedMessage).catch((e) =>
                  logger.error(e),
                );
                subHandler(connection, parsedMessage).catch((e) =>
                  logger.error(e),
                );
                bitHandler(connection, parsedMessage).catch((e) =>
                  logger.error(e),
                );
                ...
                if (
                  (!botCommand || botCommand === "ACTION") &&
                  parsedMessage.source?.nick &&
                  parsedMessage.parameters
                ) {
                  discordChatWebhook(
                    parsedMessage.source.nick,
                    Config.webhooks.discordChatHook,
                    parsedMessage.parameters,
                  );
              ...
              case "NOTICE":
                ...
                if (
                  ...
                  logger.error(
                    `Twitch IRC WebSocket: Authentication failed; left ${channel}`,
                  );
                  ...
                  logger.error(
                    `Twitch IRC WebSocket: No permission. Check if the access token is still valid. Left ${channel}`,
                  );
                  connection.send(`PART ${channel}`);
                }
                break;
...
function reconnectClient(client: websocket.client, connectionAttempts = 1) {
  ...
  setTimeout(() => {
    if (reconnecting) {
      logger.info(
        `Twitch IRC WebSocket: Reconnection attempt ${connectionAttempts}`,
      );
      client.connect(TWITCH_CHAT_IRC_WS_URL);
      connectionAttempts++;
