...
export function runTwitchEventSubWebsocket() {
  ...
  client.on("connect", function (connection) {
    ...
    connection.on("error", function (error) {
      logger.error(
        "Twitch EventSub WebSocket: Connection Error: " + error.toString(),
      );
    });
        ...
        switch (data.metadata.message_type) {
          case "session_welcome":
            {
              ...
              if (sessionId) {
                ...
                subscribeToStreamOnlineNotifications(sessionId).catch((e) =>
                  logger.error(e),
                );
                subscribeToStreamOfflineNotifications(sessionId).catch((e) =>
                  logger.error(e),
                );
              }
            }
          ...
          case "notification":
            if (isSubscriptionEvent(data.payload)) {
              ...
              twitchEventSubHandler(transformedData).catch((e) =>
                logger.error(e),
              );
            }
            break;
