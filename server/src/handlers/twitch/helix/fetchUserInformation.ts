...
export const fetchUserInformationById = async (
  userId: string,
...
export const fetchUserInformationByName = async (
  loginName: string,
): Promise<UserInformation | null> => {
  if (!loginName) {
    return null;
  ...
  try {
    ...
    if (hasOwnProperty(result, "data")) {
      ...
      if (
        ...
        hasOwnProperty(result.data[0], "display_name") &&
        typeof result.data[0].display_name === "string"
      ) {
