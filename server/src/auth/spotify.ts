...
export const getCurrentAccessToken = () =>
  getTokenFromFile("spotify_access_token");
...
export const fetchWithRetry = async (
  ...
  if (rateLimitReached) {
    throw new Error(
      "Spotify API rate limit reached. Waiting for rate limit to be lifted.",
    );
  }
  ...
  if (result.status === StatusCodes.TOO_MANY_REQUESTS) {
    ...
    if (retryAfter) {
      ...
      if (retryAfterSeconds) {
        logger.info(
          `Spotify API rate limit reached. Waiting ${retryAfterSeconds} seconds before allowing another request.`,
        );
        setTimeout(() => {
  ...
  try {
    ...
    logger.error(
      `Unable to parse JSON response from Spotify API. Error: ${errorMessage(
        error,
...
const refreshAccessToken = async (
  ...
  try {
    ...
    const url = `${SPOTIFY_AUTH_URL}api/token?client_id=${
    ...
    throw new Error(
      `Unable to use Refresh Token to obtain new Access Token from Spotify. Error: ${errorMessage(
        error,
      )}`,
    );
  }
...
const getNewAccessToken = async (
  ...
  try {
    ...
    throw new Error(
      `Unable to obtain new Access Token from Spotify. Error: ${errorMessage(
        error,
      )}`,
    );
  }
...
export const getSpotifyAccessToken = async (): Promise<string> => {
  ...
  try {
    ...
    throw new Error(
      `Unable to get Spotify Access Token. Error: ${errorMessage(error)}`,
    );
  }
};
...
export const spotifyAuthCodeRouter = async () => {
  if (Config.spotify.client_id && !Config.spotify.auth_code) {
    express()
      .get("/", (req, res) => {
        if (req.query.code) {
          ...
          res.send(
            "Hello from twitch-bot! No Spotify auth code received. You may close this window.",
          );
        }
      })
      .listen(3000);
    ...
    logger.info(
      `Getting Spotify auth code with scopes ${pc.green(
