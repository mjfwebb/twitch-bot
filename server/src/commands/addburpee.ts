...
export const addburpee: BotCommand = {
  ...
  callback: async (connection) => {
    const customReward = getCustomRewards().find((customReward) => customReward.id === burpeeRedeemId);
    ...
    if (amount) {
      ...
      const burpeeAddOneReward = getCustomRewards().find((customReward) => customReward.id === addBurpeeRedeemId);
