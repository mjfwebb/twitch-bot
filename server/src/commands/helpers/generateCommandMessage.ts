...
export const generateCommandMessage = (
  commandName: string,
  ...
  const useBy = botCommand.mustBeUser
    ? `It may only be used by ${botCommand.mustBeUser.join(",")}.`
    : "";
  const cooldown = botCommand.cooldown
    ? `It may only be used once every ${msToTimeString(botCommand.cooldown)}`
    : "";
  const aliases =
    Array.isArray(botCommand.command) && botCommand.command.length > 1
      ? `It can be invoked using ${
          botCommand.command.length
        } aliases: ${botCommand.command.join(", ")}`
      : "";
  const commandTimesUsed = `It has been used ${timesUsed} ${simplePluralise(
    "time",
    timesUsed,
  )}.`;
