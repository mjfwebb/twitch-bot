...
function sendChatQuote(connection: connection, quote: Quote) {
  sendChatMessage(
    connection,
    `Quote ${quote.quoteId}, added by ${quote.author}: ${quote.quoteText}`,
  );
}
        ...
        if (newCommandParts.length > 1) {
          ...
          switch (commandName) {
            case "add": {
              ...
              sendChatMessage(
                connection,
                `Quote ${newId} added. You can see it by using the command !quote ${newId}.`,
              );
              break;
            }
            case "delete": {
              ...
              if (!Number(quoteText)) {
                sendChatMessage(
                  connection,
                  `That doesn't seem right. To delete a quote, use the command !quote delete <quoteId>.`,
                );
                break;
              }
              ...
              if (foundQuote) {
                ...
                sendChatMessage(
                  connection,
                  `Quote ${foundQuote.quoteId} deleted.`,
                );
              } else {
                sendChatMessage(
                  connection,
                  `Unable to find a quote with the id ${quoteParams}.`,
                );
              }
            ...
            case "update": {
              ...
              if (foundQuote) {
                ...
                sendChatMessage(
                  connection,
                  `Quote number ${foundQuote.quoteId} updated.`,
                );
              }
              break;
            }
            default:
              sendChatMessage(
                connection,
                `That doesn't seem right. ${quoteHelpText}`,
              );
              break;
          }
        } else {
          if (Number(quoteParams) > 0) {
            ...
            sendChatMessage(
              connection,
              `There are ${Quotes.data.length} quotes.`,
            );
            ...
            sendChatMessage(
              connection,
              `That doesn't seem right. To get a quote, use the command !quote <quoteId>.`,
            );
          }
        }
      }
