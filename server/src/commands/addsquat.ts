...
export const addsquat: BotCommand = {
  ...
  callback: async (connection) => {
    const customReward = getCustomRewards().find((customReward) => customReward.id === squatRedeemId);
    ...
    if (amount) {
      ...
      const squatAddOneRewards = getCustomRewards().find((customReward) => customReward.id === addBurpeeRedeemId);
