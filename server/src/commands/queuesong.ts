    ...
    if (hasBotCommandParams(parsedCommand.parsedMessage)) {
      ...
      if (trackInput.startsWith(spotifyLinkUrl)) {
        ...
        if (spotifyLinkUrlResponse.status !== 200) {
          sendChatMessage(
            connection,
            `Something went wrong adding the song to the queue athanoSad. Try again?`,
          );
          return;
        }

        const htmlResponse = await spotifyLinkUrlResponse.text();
        const actionHref = parseHTML(htmlResponse)
          .window.document.querySelector("a.action")
          ?.getAttribute("href");
      ...
      if (
        !trackInput.startsWith(trackStart) &&
        !trackInput.startsWith(openSpotifyTrackUrl) &&
        !trackInput.startsWith(spotifyLinkUrl)
      ) {
        sendChatMessage(
          connection,
      ...
      if (track && !track.is_playable) {
        sendChatMessage(
          connection,
          `Song "${track.name}" is not available for me athanoSad`,
        );
        return;
      }

      // Add the track to the playback queue
      const songAddedToQueue = await addSongToPlaybackQueue(
        `${trackStart}${trackId}`,
      );
      ...
      sendChatMessage(
        connection,
        `Song "${track.name} - ${trackArtists}" added to the queue athanoCool`,
      );
    }
  },
};
