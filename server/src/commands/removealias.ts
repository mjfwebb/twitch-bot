...
export const removealias: BotCommand = {
  ...
  hidden: true,
  description:
    "Remove the alias for a command. Usage: !removealias <command> <alias>",
      ...
      if (newCommand) {
        ...
        if (newCommandParts.length > 1) {
          ...
          if (commandName === commandAlias) {
            sendChatMessage(
              connection,
              `The alias to be removed cannot be the same as the command name.`,
            );
            return;
          }
          ...
          if (!Array.isArray(command.command)) {
            sendChatMessage(
              connection,
              `The command ${commandName} does not have any aliases.`,
            );
            return;
          }
          ...
          if (Array.isArray(command.command)) {
            ...
            const aliasIndex = command.command.findIndex(
              (c) => c === commandAlias,
            );
            if (aliasIndex > -1) {
              ...
              sendChatMessage(
                connection,
                `The alias ${commandAlias} for the command ${commandName} has been removed!`,
              );
            } else {
              sendChatMessage(
                connection,
                `The command ${commandName} does not have the alias ${commandAlias}.`,
              );
            }
          }
        }
