...
export const help: BotCommand = {
  ...
  callback: (connection, parsedCommand) => {
    if (hasBotCommandParams(parsedCommand.parsedMessage)) {
      const commandToGetDetailsAbout =
        parsedCommand.parsedMessage.command?.botCommandParams?.replace("!", "");
      if (!commandToGetDetailsAbout) {
        sendChatMessage(
          connection,
          "You need to specify a command to get details about, like !help play",
        );
        return;
      }
