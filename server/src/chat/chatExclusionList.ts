...
export function loadChatExclusionList() {
  ...
  if (existsSync("chat-exclusion-list.txt")) {
    const chatExclusionListContents = readFileSync(
      "chat-exclusion-list.txt",
      "utf8",
    );
    ...
    chatExcludedUsers
      ...
      .forEach((user) => chatExclusionList.add(user.toLowerCase()));

    if (chatExclusionList.size > 0) {
      logger.debug(
        `Loaded ${chatExclusionList.size} users into chat exclusion list`,
      );
    }
  } else {
    logger.info(
      `${pc.blue("Startup:")} No chat exclusion list found, creating one`,
    );
