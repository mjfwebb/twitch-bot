...
const propertyTypes: Record<UserProperties, string> &
    ...
    for (const user of data as unknown[]) {
      ...
      for (const requiredProperty of requiredProperties) {
        if (!hasOwnProperty(user, requiredProperty)) {
          logger.error(
            `Invalid user format, missing property ${requiredProperty}`,
          );
          response = "invalid";
        }
      }
      ...
      for (const property of [...userProperties, ...timestampProperties]) {
        if (hasOwnProperty(user, property)) {
          if (typeof user[property] !== propertyTypes[property]) {
            logger.error(
              `Invalid user format, property ${property} is not of type ${propertyTypes[property]}`,
            );
            response = "invalid";
          }
        } else {
          logger.warn(
            `Invalid user format, missing property ${property}. Default value will be set.`,
          );
