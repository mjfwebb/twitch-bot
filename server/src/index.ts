...
async function main() {
  try {
    ...
    if (Config.features.commands_handler) {
      logger.info(
        `${pc.green("[Commands handler enabled]")} Loading bot commands`,
      );
    }
    ...
    if (Config.spotify.enabled) {
      logger.info(
        `${pc.green("[Spotify enabled]")} Getting Spotify access token`,
      );
      await getSpotifyAccessToken();

      logger.info(
        `${pc.green("[Spotify enabled]")} Loading Spotify bot commands`,
      );

      logger.info(
        `${pc.green("[Spotify enabled]")} Loading Spotify interval commands`,
      );
      loadSpotifyIntervalCommands();
    }
    ...
    if (Config.github.enabled) {
      logger.info(
        `${pc.green("[GitHub enabled]")} Loading GitHub bot commands`,
      );
    }
    ...
    if (customRewards.length > 0) {
      logger.debug(
        `Loaded ${pc.green(`${customRewards.length}`)} custom rewards`,
      );
      customRewards.forEach((reward) =>
        logger.debug(
          `  - ${pc.green(`${reward.title}`)} with ID ${pc.green(
            `${reward.id}`,
          )}`,
        ),
      );
    ...
    setDisplayName(
      (await fetchChannelInformation())?.broadcaster_name ||
    ...
    if (Config.features.events_handler) {
      logger.info(
        `${pc.green(
          "[Events handler enabled]",
        )} Running Twitch WebSocket client`,
      );
    ...
    if (Config.features.interval_commands) {
      logger.info(
        `${pc.green("[Interval commands enabled]")} Loading interval commands`,
      );
      loadIntervalCommands();
    }
    ...
    if (Config.betterTTV.enabled) {
      logger.info(
        `${pc.green("[BetterTTV enabled]")} Running BetterTTV WebSocket client`,
      );
    ...
    if (Config.sevenTV.enabled) {
      ...
      if (sevenTVUser) {
        ...
        logger.info(
          `${pc.green("[7TV enabled]")} Running 7TV WebSocket client`,
        );
