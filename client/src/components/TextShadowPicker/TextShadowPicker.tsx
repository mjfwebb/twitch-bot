...
export default ({ onChange, value, className = '' }: Props) => {
  ...
  return (
    <Wrapper className={'text-shadow-picker ' + className}>
      <OffsetField value={state?.offset || { x: '0', y: '0' }} onChange={updateState('offset')} />
      <SliderField value={state?.blur || '0'} onChange={updateState('blur')} title={'Blur'} />
      <ColorField value={state?.color || '#000000'} onChange={updateState('color')} />
    </Wrapper>
  );
};
